...
namespace YkinikY
{
    public class PlayerCameraFollow_ykiniky : MonoBehaviour
    {
        ...
        public float zoomedInVal = 6;
        public float zoomedOutVal = 8;
        ...
        void Update()
        {
            ...
            if (zoomed)
            {
                GetComponent<Camera>().orthographicSize = Vector2.Lerp(new Vector2(GetComponent<Camera>().orthographicSize, 0), new Vector2(zoomedInVal, 0), 0.03f).x;
            }
            else
            {
                GetComponent<Camera>().orthographicSize = Vector2.Lerp(new Vector2(GetComponent<Camera>().orthographicSize, 0), new Vector2(zoomedOutVal, 0), 0.03f).x;
            }
        }
        public void FollowX()
        {
